---
name: agent-browser
description: AI 代理的浏览器自动化 CLI。当用户需要与网站交互时使用，包括导航页面、填写表单、点击按钮、截图、提取数据、测试 Web 应用或自动化任何浏览器任务。触发条件包括请求"打开网站"、"填写表单"、"点击按钮"、"截图"、"抓取页面数据"、"测试此 Web 应用"、"登录网站"、"自动化浏览器操作"或任何需要编程化 Web 交互的任务。
...
agent-browser type @e2 "text"         # 输入文本而不清除
...
**要求：** macOS 及 Xcode、Appium（`npm install -g appium && appium driver install xcuitest`）
