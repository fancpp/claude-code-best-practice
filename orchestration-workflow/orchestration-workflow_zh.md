...
本文档描述了 **命令 → 代理（带技能）→ 技能** 的编排工作流，通过一个天气数据获取和 SVG 渲染系统进行演示。
...
天气系统在单个编排工作流中展示了两种不同的技能模式：
- **代理技能**（预加载）：`weather-fetcher` 在启动时被注入到 `weather-agent` 中作为领域知识
...
该代理在启动时将 `weather-fetcher` 预加载到其上下文中。它遵循技能的指令获取温度，并将值返回给命令。
...
- **调用方式**：通过命令中的 Skill 工具调用（未预加载到任何代理中）
- **输出**：
...
- **输出**：温度值和单位（摄氏度或华氏度）
- **说明**：这是一个代理技能 — 预加载到 `weather-agent` 中，不直接调用
...
5. **SVG 创建**：命令通过 Skill 工具调用 `weather-svg-creator`
   ...
   - 技能将概要写入 `orchestration-workflow/output.md`
6. **结果展示**：向用户显示概要，包含：
   - 请求的温度单位
   - 获取的温度
   - SVG 卡片位置
...
1. **两种技能模式**：展示代理技能（预加载）和技能（直接调用）
2. **命令作为编排器**：命令处理用户交互并协调工作流
3. **代理负责数据获取**：代理使用其预加载的技能获取数据，然后返回
4. **技能负责输出**：SVG 创建器独立运行，从命令上下文接收数据
5. **清晰分离**：获取（代理）→ 渲染（技能）— 每个组件承担单一职责
...
- **技能预加载**：完整的技能内容在启动时注入到代理的上下文中
- **代理使用技能知识**：代理遵循预加载技能中的指令
- **无需动态调用**：技能是参考材料，不单独调用
