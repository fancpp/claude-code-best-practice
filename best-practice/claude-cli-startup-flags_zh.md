...
从终端启动 Claude Code 时的 Claude Code 启动标志、顶级子命令和启动环境变量参考。
...
| `--allow-dangerously-skip-permissions` | | 启用权限绕过作为选项（但不激活） |
...
| `--permission-prompt-tool <TOOL>` | | 指定 MCP 工具以在非交互模式下处理权限提示 |
...
| `--json-schema <SCHEMA>` | | 获取符合模式的已验证 JSON（仅打印模式） |
...
| `--system-prompt-file <PATH>` | | 从文件加载系统提示，替换默认提示（仅打印模式） |
...
| `--plugin-dir <PATH>` | | 仅本次会话从目录加载插件（可重复） |
...
| 标志 | 简写 | 描述 |
|------|-------|-------------|
| `--max-budget-usd <AMOUNT>` | | 停止前 API 调用的最大美元金额（仅打印模式） |
...
| `--no-chrome` | | 禁用本次会话的 Chrome 浏览器集成 |
...
| 标志 | 简写 | 描述 |
|------|-------|-------------|
| `--debug <CATEGORIES>` | | 启用调试模式，可选择分类过滤（例如 `"api,hooks"`） |
...
| `--setting-sources <LIST>` | | 逗号分隔的加载源列表：`user`、`project`、`local` |
| `--disable-slash-commands` | | 禁用本次会话的所有技能和斜杠命令 |
...
这些启动专用的环境变量在启动 Claude Code 之前在 Shell 中设置（它们无法通过 `settings.json` 配置）：
...
| `CCR_FORCE_BUNDLE=1` | 使用 `claude --remote` 时强制打包/上传本地仓库。也可通过 `env` 键配置 — 参见 [Settings Reference](./claude-settings.md#environment-variables-via-env) |
